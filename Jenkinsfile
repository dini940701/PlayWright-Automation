...
pipeline 
{
	...
	environment {
		...
		// Git Bash path used for unix-style commands on Windows
	...
	stages {
		...
		stage('🔍 ESLint Analysis') {
			steps {
				...
				echo '============================================'
				bat "${GIT_BASH} \"mkdir -p eslint-report\""
				...
				echo '============================================'
				bat "${GIT_BASH} \"npm run lint:report || true\""
			}
			post {
				always {
					publishHTML(target: [
		...
		stage('🔧 DEV Tests') {
			steps {
				...
				echo '============================================'
				bat "${GIT_BASH} \"npx playwright install --with-deps chromium\""

				echo '============================================'
				echo '🧹 Cleaning previous results...'
				echo '============================================'
				bat "${GIT_BASH} \"rm -rf allure-results playwright-report playwright-html-report test-results\""
				...
				script {
					env.DEV_TEST_STATUS = bat(
						script: "${GIT_BASH} \"npx playwright test tests/loginPage.spec.ts --config=playwright.config.dev.ts\"",
						returnStatus: true
					) == 0 ? 'success' : 'failure'
				}
				...
				echo '============================================'
				bat "${GIT_BASH} \"mkdir -p \\\"allure-results\\\" && echo 'Environment=DEV' > \\\"allure-results/environment.properties\\\" && echo 'Browser=Google Chrome' >> \\\"allure-results/environment.properties\\\" && echo 'Config=playwright.config.dev.ts' >> \\\"allure-results/environment.properties\\\"\""
			}
			post {
				always {
					bat "${GIT_BASH} \"mkdir -p allure-results-dev && cp -r allure-results/* allure-results-dev/ 2>/dev/null || true && npx allure generate allure-results-dev --clean -o allure-report-dev || true\""
		...
		stage('🔧 QA Tests') {
			steps {
				...
				script {
					env.QA = bat(
						script: "${GIT_BASH} \"npx playwright test tests/loginPage.spec.ts --config=playwright.config.qa.ts\"",
						returnStatus: true
					) == 0 ? 'success' : 'failure'
				}
				...
				echo '🏷️ Adding Allure environment info...'
				echo '============================================'
				bat "${GIT_BASH} \"mkdir -p allure-results && echo 'Environment=QA' > allure-results/environment.properties && echo 'Browser=Google Chrome' >> allure-results/environment.properties && echo 'Config=playwright.config.qa.ts' >> allure-results/environment.properties\""
			}
			post {
				always {
					bat "${GIT_BASH} \"mkdir -p allure-results-qa && cp -r allure-results/* allure-results-qa/ 2>/dev/null || true && npx allure generate allure-results-qa --clean -o allure-report-qa || true\""
		...
		stage('🔧 STAGE Tests') {
			steps {
				...
				script {
					env.STAGE = bat(
						script: "${GIT_BASH} \"npx playwright test tests/loginPage.spec.ts --config=playwright.config.stage.ts\"",
						returnStatus: true
					) == 0 ? 'success' : 'failure'
				}
				...
				echo '🏷️ Adding Allure environment info...'
				echo '============================================'
				bat "${GIT_BASH} \"mkdir -p allure-results && echo 'Environment=STAGE' > allure-results/environment.properties && echo 'Browser=Google Chrome' >> allure-results/environment.properties && echo 'Config=playwright.config.stage.ts' >> allure-results/environment.properties\""
			}
			post {
				always {
					bat "${GIT_BASH} \"mkdir -p allure-results-stage && cp -r allure-results/* allure-results-stage/ 2>/dev/null || true && npx allure generate allure-results-stage --clean -o allure-report-stage || true\""
		...
		stage('🔧 PROD Tests') {
			steps {
				...
				script {
					env.PROD = bat(
						script: "${GIT_BASH} \"npx playwright test tests/loginPage.spec.ts --config=playwright.config.prod.ts\"",
						returnStatus: true
					) == 0 ? 'success' : 'failure'
				}
				...
				echo '🏷️ Adding Allure environment info...'
				echo '============================================'
				bat "${GIT_BASH} \"mkdir -p allure-results && echo 'Environment=PROD' > allure-results/environment.properties && echo 'Browser=Google Chrome' >> allure-results/environment.properties && echo 'Config=playwright.config.prod.ts' >> allure-results/environment.properties\""
			}
			post {
				always {
					bat "${GIT_BASH} \"mkdir -p allure-results-prod && cp -r allure-results/* allure-results-prod/ 2>/dev/null || true && npx allure generate allure-results-prod --clean -o allure-report-prod || true\""
		...
		stage('📈 Combined Allure Report') {
			steps {
				...
				bat "${GIT_BASH} \"mkdir -p allure-results-combined\""
				bat "${GIT_BASH} \"cp -r allure-results-dev/* allure-results-combined/ 2>/dev/null || true\""
				bat "${GIT_BASH} \"cp -r allure-results-qa/* allure-results-combined/ 2>/dev/null || true\""
				bat "${GIT_BASH} \"cp -r allure-results-stage/* allure-results-combined/ 2>/dev/null || true\""
				bat "${GIT_BASH} \"cp -r allure-results-prod/* allure-results-combined/ 2>/dev/null || true\""
				bat "${GIT_BASH} \"echo \\\"Environment=ALL (DEV, QA, STAGE, PROD)\\\" > allure-results-combined/environment.properties\""
				bat "${GIT_BASH} \"echo \\\"Browser=Google Chrome\\\" >> allure-results-combined/environment.properties\""
				bat "${GIT_BASH} \"echo \\\"Pipeline=${JOB_NAME}\\\" >> allure-results-combined/environment.properties\""
				bat "${GIT_BASH} \"echo \\\"Build=${BUILD_NUMBER}\\\" >> allure-results-combined/environment.properties\""
			}
			post {
				always {
					allure([
