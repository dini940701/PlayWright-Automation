...
def notifySlackAndEmail(status) {
    def statusText = status=='passed'?'✅ Playwright Pipeline: All Tests Passed':status=='failed'?'❌ Playwright Pipeline: Tests Failed':'⚠️ Playwright Pipeline: Unstable'
...
<head>
    <style>
        ...
        .header { background: ${status=='passed'?'#27ae60':status=='failed'?'#e74c3c':'#f39c12'}; color: white; padding: 20px; text-align: center; border-radius: 8px 8px 0 0; }
...
<body>
    <div class="container">
        ...
        <div class="content">
            ...
            <a href="${env.BUILD_URL}allure" class="btn btn-purple">Combined Allure Dashboard</a>
            <a href="${env.BUILD_URL}" class="btn btn-green">View Build</a>
